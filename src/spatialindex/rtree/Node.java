...
abstract class Node implements INode
{
	...
	protected int m_parentNodeIdentifier = 0;
		// Parent of this node.
...
//	public HashMap<Integer,Node> children = new HashMap<>();
	...
	protected HashMap<String,ArrayList> m_invertedList;
	...
	protected Node(RTree pTree, int id, int level, int capacity)
	{
		...
		m_invertedList = new HashMap<String,ArrayList>();
	}
...
//	}
	...
	public void BuildInvertedFile()
	{
//		System.out.println("build inverted");
		//对每一个节点，都建立一个buckets -> object/node 列表
		if(isLeaf())//如果是叶子节点
		{
//			System.out.println("in leaf");
			...
			for(int i=0 ; i<m_children ; i++)
  		    {
				int id = m_pIdentifier[i];
				//在这里能够获取叶节点中对象的id；通过id在hashmap中找到相应的桶号，并将桶号以及id加入到这个叶节点的倒排文件当中去
				//需要确定的是bucketnumber中是否存储了桶号
				ArrayList<String> bucket = newTest.map.get(id).bucketnumber;
//				System.out.println("content of bucketnumber:");
//				for(int p=0;p<bucket.size();++p){
//					System.out.println(" "+bucket.get(p));
//				}
//				System.out.println();
				if(m_invertedList != null)
				{
					for(int i2=0; i2<bucket.size(); i2++)
					{
						if(!m_invertedList.containsKey(bucket.get(i2))){
							ArrayList temp = new ArrayList();
							temp.add(id);
							m_invertedList.put(bucket.get(i2), temp);
						}else{//若是已经包含了此关键词，则加入到已存在的数组中
							ArrayList temp = m_invertedList.get(bucket.get(i2));
							temp.add(id);
							m_invertedList.put(bucket.get(i2), temp);
						}
//						System.out.println("object id:"+id);
		...
		{
//			System.out.println("in non leaf");
			m_invertedList = new HashMap<String,ArrayList>();
			
			
			for(int i=0 ; i<m_children ; i++)
			{
				Node n = m_pTree.readNode(m_pIdentifier[i]);
//				this.children.put(m_pIdentifier[i], n);
				n.BuildInvertedFile();
				Iterator<String> keys = n.m_invertedList.keySet().iterator();
				while(keys.hasNext()){
					String key = keys.next();//bucketid
					if(m_invertedList.get(key)==null){
						ArrayList list = new ArrayList();
						list.add(n.m_identifier);
						m_invertedList.put(key, list);
					}else{
						m_invertedList.get(key).add(n.m_identifier);
					}
//					System.out.println("node id:"+n.m_identifier);
				}
			}
		}
		newTest.inverted.put(this.m_identifier, this.m_invertedList);
//		System.out.print("node id"+this.m_identifier );
//		Iterator<String> iterator = m_invertedList.keySet().iterator();
...
//		System.out.println(m_invertedList);
	}
